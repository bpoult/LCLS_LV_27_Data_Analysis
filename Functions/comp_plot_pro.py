...
def plot_lots_comp(pro_datas1,input_vars1,pro_datas,input_vars):
    pro_datas1 = pro_datas1
    scans_to_plot_comp = input_vars1[0]
    spec_cal_runs_comp = input_vars1[1]
    xrt_plot = input_vars1[2][1]
    epix_plot = input_vars1[2][0]
    resid_plot = input_vars1[2][2]
    deltaT_T_plot = input_vars1[2][3]
    plot_shotwise = input_vars1[3]
    plot_meanwise = input_vars1[4]
    runs_comp = input_vars1[5]
    bootstrapped = input_vars1[6]
    energy = pro_datas1[0].epix_energy_windowed
...
#     make second set of variables and plot things
    ...
    print('spec runs',spec_cal_runs,runs)
    ...
    if epix_plot:
        ...
        [plt.plot(energy,epix_means_comp[i]) for i in range(0,len(epix_means_comp))]
        plt.legend(([scans_to_plot[i] for i in range(0,len(epix_means))],[scans_to_plot_comp[j] for j in range(0,len(epix_means_comp))]))
        plt.title('Epix Spectra Stacked')
        plt.xlabel('energy, keV')
        txt = (str(scans_to_plot[0])+' calibration: '+str(spec_cal_runs[0])+' and '+str(scans_to_plot_comp[0])+' calibration: '+str(spec_cal_runs_comp[0]))
    ...
    if xrt_plot:
        ...
        [plt.plot(energy,xrt_means_comp[i]) for i in range(0,len(xrt_means_comp))]
        plt.legend((scans_to_plot[0],scans_to_plot_comp[0]))    
    ...
    if plot_meanwise:
        ...
        if resid_plot:
            ...
            [plt.plot(energy,resid_means_comp[i]) for i in range(0,len(resid_means_comp))]
            plt.legend((scans_to_plot[0],scans_to_plot_comp[0]))    
        ...
        if deltaT_T_plot:
            ...
            [plt.plot(energy,deltaT_T_means_comp[i]) for i in range(0,len(deltaT_T_means_comp))]
            plt.legend((scans_to_plot[0],scans_to_plot_comp[0]))    
    ...
    if plot_shotwise:
        ...
        if resid_plot:
            ...
            [plt.plot(energy,np.mean(resid_sbs_comp[i],0)) for i in range(0,len(resid_sbs_comp))]
            plt.legend((scans_to_plot[0],scans_to_plot_comp[0]))    
        ...
        if deltaT_T_plot:
            ...
            [plt.plot(energy,np.mean(deltaT_T_sbs_comp[i],0)) for i in range(0,len(deltaT_T_sbs_comp))]
            plt.legend((scans_to_plot[0],scans_to_plot_comp[0]))    
