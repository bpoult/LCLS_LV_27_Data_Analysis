...
#     pro_datas = [pro_datas0,pro_datas1,pro_datas2]
    ...
    scans_to_plot_comp = input_vars[0]
    spec_cal_runs_comp = input_vars[1]
    runs_comp = input_vars[2]
    energy_comp = pro_datas[1][0].epix_energy_windowed
...
#     print(type(scans_to_plot_comp))
#     print(type(pro_datas1))
#     print(type(runs_comp))
...
#     make another set of variables
    pro_datas2 = pro_datas[2]
    scans_to_plot_comp2 = input_vars[3]
    spec_cal_runs_comp2 = input_vars[4]
    runs_comp2 = input_vars[5]
    energy_comp2 = pro_datas[2][0].epix_energy_windowed
...
#     make another set of variables and plot things
    scans_to_plot = input_vars[6]
    spec_cal_runs = input_vars[7]
    xrt_plot = input_vars[8][1]
    epix_plot = input_vars[8][0]
    resid_plot = input_vars[8][2]
    deltaT_T_plot = input_vars[8][3]
    plot_shotwise = input_vars[8]
    plot_meanwise = input_vars[8]
    runs = input_vars[9]
    bootstrapped = input_vars[10]
    pro_datas0 = pro_datas[0]
    energy = pro_datas0[0].epix_energy_windowed

    print(type(pro_datas0))
    print(type(pro_datas1))
    print(type(pro_datas2))
    
#     shots_in_scale = gather_shots(pro_datas0,[spec_cal_runs,1,runs])
#     scale = np.mean(np.mean(shots_in_scale[1],0),0)/np.mean(np.mean(shots_in_scale[0],0),0)

#     compare = np.asarray(gather_shots(pro_datas0,[scans_to_plot,1,runs]))
...
#     xrt_means = [np.mean(scale*compare[0][i],0) for i in range(0,compare[0].shape[0])]
    ...
    padding = 3
#     cal '+str(spec_cal_runs[0])+' on ',[str(scans_to_plot[i]) for i in range(0,len(epix_means))],'
#     txt = 'cal ' + str(spec_cal_runs_comp[0])+' on ',[str(scans_to_plot_comp[i]) for i in range(0,len(epix_means_comp))]
    txt = ' '
...
#         [plt.plot(energy,epix_means[i],label = scans_to_plot[i]) for i in range(0,len(epix_means))]
        [plt.plot(energy_comp,epix_means_comp[i],label = scans_to_plot_comp[i]) for i in range(0,len(epix_means_comp))]
        [plt.plot(energy_comp2,epix_means_comp2[i],label = scans_to_plot_comp2[i]) for i in range(0,len(epix_means_comp2))]
        plt.legend()
...
#         [plt.plot(energy,xrt_means[i],label = scans_to_plot[i]) for i in range(0,len(xrt_means))]
        [plt.plot(energy_comp,xrt_means_comp[i],label = scans_to_plot_comp[i]) for i in range(0,len(xrt_means_comp))]
        [plt.plot(energy_comp2,xrt_means_comp2[i],label = scans_to_plot_comp2[i]) for i in range(0,len(xrt_means_comp2))]
        plt.legend()    
...
#             [plt.plot(energy,resid_means[i],label = scans_to_plot[i]) for i in range(0,len(resid_means))]
            [plt.plot(energy_comp,resid_means_comp[i],label = scans_to_plot_comp[i]) for i in range(0,len(resid_means_comp))]
            [plt.plot(energy_comp2,resid_means_comp2[i],label = scans_to_plot_comp2[i]) for i in range(0,len(resid_means_comp2))]
            plt.legend()    
...
#             [plt.plot(energy,deltaT_T_means[i],label = scans_to_plot[i]) for i in range(0,len(deltaT_T_means))]
            [plt.plot(energy_comp,deltaT_T_means_comp[i],label = scans_to_plot_comp[i]) for i in range(0,len(deltaT_T_means_comp))]
            [plt.plot(energy_comp2,deltaT_T_means_comp2[i],label = scans_to_plot_comp2[i]) for i in range(0,len(deltaT_T_means_comp2))]
            plt.legend()    
...
#             [plt.plot(energy,np.mean(resid_sbs[i],0),label = scans_to_plot[i]) for i in range(0,len(resid_sbs))]
            [plt.plot(energy_comp,np.mean(resid_sbs_comp[i],0),label = scans_to_plot_comp[i]) for i in range(0,len(resid_sbs_comp))]
            [plt.plot(energy_comp2,np.mean(resid_sbs_comp2[i],0),label = scans_to_plot_comp2[i]) for i in range(0,len(resid_sbs_comp2))]
            plt.legend()    
...
#             [plt.plot(energy,np.mean(deltaT_T_sbs[i],0),label = scans_to_plot[i]) for i in range(0,len(deltaT_T_sbs))]
            [plt.plot(energy_comp,np.mean(deltaT_T_sbs_comp[i],0),label = scans_to_plot_comp[i]) for i in range(0,len(deltaT_T_sbs_comp))]
            [plt.plot(energy_comp2,np.mean(deltaT_T_sbs_comp2[i],0),label = scans_to_plot_comp2[i]) for i in range(0,len(deltaT_T_sbs_comp2))]
            plt.legend()    
