...
#ds = ps.DataSource(dir)
    ignore_runs = set(list(range(0,170)))
    ...
    for i in range(0,len(newList)):
        ...
        if not np.logical_or(os.path.exists(load_data_input[0] + scan_name +'/scratch.txt'),os.path.exists(load_data_input[0] + scan_name +'/rawdata.pkl')):
